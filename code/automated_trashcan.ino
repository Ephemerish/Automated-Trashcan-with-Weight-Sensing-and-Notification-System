  ...
  Description:
    ...
    does not exceeds a preset threshold and a person is detected nearby, the trashcan automatically opens for 
  ...
  */////////////////////////////////////////////////////////////////////////////
  ...
  // Wifi manager (Storage File Name)
  ...
  // Constants
  const int MAX_DISTANCE = 200; // centimeters
  const float WEIGHT_THRESHOLD = 1000.0; // grams
  const unsigned long INTERVAL_MS = 0; // milliseconds
  const int CALIBRATION_EEPROM_ADDRESS = 0;
  const unsigned long LONG_PRESS_THRESHOLD = 300; // milliseconds
  const int OPEN_SERVO_ANGLE = 0; // degrees
  const int CLOSE_SERVO_ANGLE = 90; // degrees
